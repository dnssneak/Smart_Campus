// ===== NOTIFICATION BELL HELPER =====
// This file provides notification bell functionality for all pages
...
async function loadBellDropdown() {
    try {
        ...
        list.innerHTML = notifications.slice(0, 5).map(n => `
            <div class="bell-dropdown-item ${n.is_read ? '' : 'unread'}" onclick="window.location.href='notifications.html'">
                ...
                <div style="flex:1">
                    ...
                    <small style="color:#94a3b8">${getTimeAgo(n.created_at)}</small>
                </div>
            </div>
        `).join('');
        ...
        const list = document.getElementById('bellDropdownList');
        if (list) {
            list.innerHTML = `<p style="padding:1.5rem;text-align:center;color:#ef4444">Failed to load</p>`;
        }
    }
}
